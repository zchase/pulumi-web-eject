...
import Table from './components/pulumiActionTable.js';
...
const Destroy: React.FC<DestroyProps> = ({ cliArgs }) => {
    ...
    useEffect(() => {
        ...
        const destroyStack = async () => {
            ...
            // Run the update.
            setStep(4);
            await pulumiRunner.destroyStack(stack, (msg: string) => {
    ...
    return(
        <Box flexDirection="column">
            <Box flexDirection="column" marginBottom={1}>
                ...
                <StatusSpinner inProgress={step === 4} done={step > 4} text="Destroy Website" />
            </Box>

            <Box display={ step > 3 ? "flex" : undefined }>
                <Table done={step === 5} data={tableData} columns={["name", "resource", "status"]} />
            </Box>
        </Box>
    );
};
